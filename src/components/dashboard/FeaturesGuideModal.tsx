...
export default function FeaturesGuideModal({ isOpen, onClose }: FeaturesGuideModalProps) {
    ...
    return createPortal(
        ...
        >
            <div
                className="w-full max-w-4xl bg-[#0a0e27] border border-[#1e2749] rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh] relative animate-scale-in"
                onClick={e => e.stopPropagation()}
            >

                {/* Hero Header */}
                <div className="relative h-48 bg-gradient-to-br from-primary-900 via-[#0a0e27] to-[#0a0e27] flex items-center px-10 border-b border-[#1e2749] shrink-0">
                    <div className="absolute inset-0 bg-grid-white/[0.05]" />
                    {/* Decorative glows */}
                    <div className="absolute top-0 right-0 w-48 h-48 bg-primary-500/20 blur-3xl rounded-full" />

                    <div className="relative z-10 max-w-2xl">
                        <h2 className="text-4xl font-display font-bold text-white mb-3">Welcome to ApicBooks</h2>
                        <p className="text-lg text-slate-300">Your intelligent reading companion. Explore all the features designed to help you read more and read better.</p>
                    </div>
                    <button
                        onClick={onClose}
                        className="absolute top-6 right-6 p-2 bg-black/20 hover:bg-white/10 rounded-full text-white/50 hover:text-white transition-colors z-20"
                    >
                        <X className="w-6 h-6" />
                    </button>
                </div>
                ...
                <div className="p-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto custom-scrollbar">
                    {features.map((f, i) => (
                        <div key={i} className="flex flex-col gap-4 p-5 rounded-2xl hover:bg-[#141b3d]/50 transition-all duration-300 border border-transparent hover:border-[#1e2749] group hover:-translate-y-1">
                ...
                {/* Footer */}
                <div className="p-6 border-t border-[#1e2749] bg-[#0d1233] flex justify-between items-center shrink-0">
